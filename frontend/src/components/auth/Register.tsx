...
export default function Register({ onNext, onBack }: RegisterProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await onNext({ fullName: fullName.trim(), email: email.trim() || undefined, phone: phone ? `+880${phone}` : undefined, password, role });
    } catch (err) {
      setErrors({ form: err instanceof Error ? err.message : 'Could not create your account. Please try again.' });
    } finally {
      setSubmitting(false);
    }
  };
      ...
      >
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <fieldset>
              <legend className="mb-2 text-sm text-gray-700">I am signing up as</legend>
              <div className="grid grid-cols-2 gap-3" role="radiogroup" aria-label="Account type">
                {([['senior', '🧓 Senior', 'I need care & support'], ['family', '👨‍👩‍👧 Family member', 'I look after a loved one']] as const).map(([value, title, hint]) => (
                  <button
                    key={value}
                    type="button"
                    role="radio"
                    ...
                    className={`p-3 rounded-xl border-2 text-left transition-all ${role === value ? 'border-purple-600 bg-purple-50' : 'border-gray-200 bg-white hover:border-purple-300'}`}
                  >
            ...
            >
                {errors.form && (
                  <div role="alert" className="mb-3 p-3 rounded-lg bg-red-50 border border-red-200 text-sm text-red-700">{errors.form}</div>
                )}
