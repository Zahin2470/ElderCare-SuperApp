...
interface VerifyPhoneProps {
  ...
  /** Only present when the API runs outside production, so local dev needs no SMS gateway. */
  devOtp?: string;
  onVerified: (session: Session) => void;
  onBack?: () => void;
}

export default function VerifyPhone({ phone, purpose = 'verify', devOtp: initialDevOtp, onVerified, onBack }: VerifyPhoneProps) {
  ...
  const handleVerify = async () => {
    ...
    setError(false);
    try {
      // The server checks the code: 5-minute expiry, 3 attempts, then the code is burned.
      const session = await post<Session>('/auth/verify-otp', { target: phone, code: otp, purpose });
      setSuccess(true);
      toast.success(isEmail ? 'Email verified successfully!' : 'Phone verified successfully!');
      setTimeout(() => onVerified(session), 800);
    } catch (e) {
      setError(true);
      setOtp('');
      const used = attempts + 1;
      setAttempts(used);
      if (used >= 3 || (e instanceof ApiError && e.status === 429)) {
        setIsLocked(true);
        toast.error('Too many attempts. Please request a new code.');
      } else {
        toast.error(errorMessage(e, 'Invalid code. Please try again.'));
      }
    } finally {
  ...
  const handleResend = async () => {
    setCanResend(false);
    try {
      const r = await post<{ devOtp?: string }>('/auth/request-otp', { target: phone, purpose });
      setTimeLeft(60);
      setOtp('');
      setError(false);
      setAttempts(0);
  ...
  const maskedPhone = isEmail ? phone.replace(/^(.{2}).*(@.*)$/, '$1***$2') : phone.replace(/(\+880)(\d{2})(\d+)(\d{2})$/, '$1$2*****$4');
      ...
      >
        ...
        >
          ...
          <div className="mb-8">
            ...
            <h1 className="text-gray-900 mb-2 text-center">{isEmail ? 'Verify your email ✉️' : 'Verify your phone 📱'}</h1>
            <p className="text-gray-600 text-center">
              We sent a 6-digit code to <br />
              <span className="font-medium text-purple-600">{maskedPhone}</span>
            </p>
            {/* Shown only when the API is not in production and returns the code (no SMS gateway in local dev). */}
            {import.meta.env.DEV && devOtp && (
              <p className="mt-3 text-xs text-center text-blue-800 bg-blue-50 border border-blue-200 rounded-lg p-2">
