...
interface ForgotPasswordProps {
  onBack: () => void;
  /** `target` is the normalised email/phone the code was sent to. */
  onSent: (target: string, devOtp?: string) => void;
}
...
export default function ForgotPassword({ onBack, onSent }: ForgotPasswordProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validate()) return;
    setIsSending(true);
    const target = method === 'email' ? email.trim() : `+880${phone}`;
    try {
      // Always succeeds for well-formed input, whether or not the account exists (prevents account enumeration).
      const r = await post<{ devOtp?: string }>('/auth/forgot-password', { identifier: target });
      onSent(target, r.devOtp);
    } catch (err) {
