...
interface LoginProps {
  /** Should reject with an Error whose message is safe to show the user. */
  onLogin: (data: { identifier: string; password?: string; useOTP?: boolean }) => Promise<void>;
  onRegister?: () => void;
  onForgotPassword?: () => void;
  onStaffLogin?: () => void;
}

export default function Login({ onLogin, onRegister, onForgotPassword, onStaffLogin }: LoginProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      if (loginMethod === 'password') await onLogin({ identifier: identifier.trim(), password });
      else await onLogin({ identifier: `+880${phone}`, useOTP: true });
    } catch (err) {
      setErrors({ form: err instanceof Error ? err.message : 'Could not sign in. Please try again.' });
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="min-h-screen relative overflow-hidden bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 flex items-center justify-center p-4">
      ...
      >
        ...
        >
          ...
          {/* Login Method Tabs */}
          {errors.form && (
            <div role="alert" className="mb-4 p-3 rounded-lg bg-red-50 border border-red-200 text-sm text-red-700">{errors.form}</div>
          )}
            ...
            <TabsContent value="password">
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <motion.div
                  ...
                  transition={{ delay: 0.2 }}
                >
                  <Button type="submit" disabled={submitting} className="w-full h-12 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white shadow-lg shadow-purple-500/30 transition-all">
                    Sign in
                  </Button>
                </motion.div>
              </form>
          ...
          {onStaffLogin && (
            <p className="text-center text-xs text-gray-500 mt-3">
              <button type="button" onClick={onStaffLogin} className="hover:text-purple-700 hover:underline">Staff sign-in</button>
            </p>
          )}
          ...
          {import.meta.env.DEV && (
            <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-xl">
              <p className="text-xs text-blue-900 mb-2"><strong>💡 Dev demo accounts</strong> (seeded data)</p>
              <p className="text-xs text-blue-800 font-mono">
                senior: demo@eldercare.com<br />
                family: family@eldercare.com<br />
                password: Demo@12345
              </p>
            </div>
          )}
